...
function CourseTemplate({title,desc,teacher,time,cost}) {
  return (
    <div className='relative rounded-xl p-2 z-2'>
      <div className='bg-white shadow shadow-blue-200 rounded-2xl h-[85%] absolute bottom-0 right-0 w-[100%] -z-1' ></div>
        <div className='mx-auto overflow-hidden w-[95%] object-center rounded-2xl bg-red-800'>
          <img src={cartImage}  alt="cart-img" />
        </div>
        <h1 className='my-3 font-sans text-[16px] '> {title} </h1>
        <p className='font-yekan text-justify px-1 mb-3 text-[14px] '> {desc} </p>
